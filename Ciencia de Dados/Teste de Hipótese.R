...
liberdade no numerados e 10 graus de liberdade no denominador
...
Análise Estatística de Dados - Página 106 de 177
...
n1 <- 25 #Armazena a quantidade observações registradas para de vendas naposição A
n2 <- 30 #Armazena a quantidade observações registradas para de vendas naposição B
...
aleatoriamente poderemos ter curvas diferentes para um mesmo valor de n, mas a
...
install.packages().
#Uma vez instalada não há mais necessidade de instalar novamente. Basta carregar
com o comando library()
install.packages('truncnorm')
library(truncnorm)
set.seed(100)
#Gera uma amostra aleatória, seguindo uma distribuição normal cujo valor mínimo é
100 e o valor máximo é 140.
...
tcrítico_teste_t_pareado <- -qt(p = 0.9, df = 19) #Devido ao teste ser unilateral aesquerda a distribuição t ser simétrica, nossa estatística de teste será negativa
...
tabela
barplot(tabela)
#O valor críticico para uma distribuição qui-quadrado com (linhas-1)*(colunas-1)=1
grau de liberdade ao nível de confiança de 95%
qchisq(p=0.95,df = 1)
...
#Assim como fizemos no test t, podemos usar um comando direto no R para realizaro teste qui-quadrado chisq.test()
...
#H1: Há diferença no valor médio gasto com bebidas em pelo menos uma daspopulações
rm(list = ls())
...
summary(anova)
# O valor p é praticamente zero. Mesmo que nosso nível de confiança fosse 99,9%
ainda teríamos evidências para rejeitar H0